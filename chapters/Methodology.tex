...
\tp{You start out here discussing cascades, but I don't remember that you introduce them. A little later in this chapter you mention some of the properties, but I think you have to give some introduction before you use the term. You can do that here, or also in the theory chapter, where you could add a section on strange baryons, their quantum numbers and their decay topologies. You could also add a sketch of these topologies, which might help in explaining some for the cut parameters like DCA,  pointing angle etc. (This latter piece probably doesn't fit into the theory chapter, so may be here is a good place. On the other hand, the quantum numbers and quark content may be good to discuss in chapter 2, before you present figures like 2.10.)}\rs{reply, and todo: I've added sections to the theory chapter, I need to revise this introduction to reduce the duplicate info.}
...
The Opposite-Sign (OS) correlation functions ($C^{+,-}$ and $C^{-,+}$) contain not only cascade pairs originating from the same hadronisation process, \tp{It may be obvious to you now, but for the non-specialist reader you may want to say a word on why you expect the particles from the same process to show up here.}\rs{reply: I've added some context on this in the theory chapter where I discuss the run 2 correlation results. this discussion is now a little bit duplicate, but I think it's still good to keep here as it's important} but also cascades that have been produced independently of each other and only belong to the same event by chance. These ``uncorrelated" pairs form a combinatorial background that needs to be accounted for in order to study the physical correlations. We do this by subtracting the Same-Sign (SS) correlation functions ($C^{+,+}$ and $C^{-,-}$) from the OS correlation functions, as the SS correlation functions are expected to only contain pairs that are produced in separate hadronisation processes. It is important to realise that there may still be physical correlations present due to other factors such as the event shape (e.g. in an event with one or multiple jets one may find angular correlations between two cascades unrelated to the hadronisation process). However, these correlations are not the object of our study, and as they are expected to be charge independent they will be properly accounted for through the SS subtraction. 

As we are interested in the balancing of strangeness and baryon number, we normalise to the number of triggers to obtain a per-trigger yield. This ``divides out'' any mechanisms related to the production of the trigger cascade, leaving us with the correlations related to the production of the associate cascade given a trigger cascade. \tp{This sounds like a logical deduction, i.e. because we want a certain result (1), we do this (2). But it is by no means obvious on how 1 follows from 2.}\rs{added a small clarifying sentence} We normalise the two charges separately to account for any difference in the yields. Note that cascades that are not matched to another cascade \textit{do} contribute to the normalisation via $N^-$ and $N^+$. By combining the SS subtraction with the per-trigger normalisation we obtain the Balance Function (BF):
\begin{align}
...
\section{Datasets \& event selections}
  \label{sec:datasets}

  For this analysis we use the \texttt{LHC24\_pass1\_MinBias} dataset, which consists of roughly $1.5 \times 10^{10}$ pp collisions (before event selections) at $\sqrt{s} = 13.6$ TeV recorded during the 2024 data taking period. We also use two datasets consisting of pp collisions simulated by PYTHIA~\cite{PythiaManual,PythiaCode} and propagated through the detector simulated using GEANT4~\cite{GEANT4}: \texttt{LHC24f4d} and \texttt{LHC24j7}. \texttt{LHC24f4d} is a ``general purpose'' Monte-Carlo (MC) dataset anchored to the 2024 data taking period, which means that the conditions and detector state were simulated to match those during the data taking as closely as possible. The \texttt{LHC24j7} dataset is also anchored to the 2024 data taking period, but it is a ``strangeness enriched'' dataset, meaning that cascades were manually added to the generated events to drastically increase the number of cascades per event (or rather reduce the number of events without cascades). This makes it much more efficient to analyze, both computationally speaking as well as regarding the disk size the data occupies. We use the \texttt{LHC24j7} dataset to determine the reconstruction efficiency as described in Section~\ref{sec:efficiency}, while we use the \texttt{LHC24f4d} dataset to perform MC closure tests as described in Appendix~\ref{ch:mc_closure}. We do not use the strangeness enriched dataset for the MC closure test as we do not know the effect (if any) the manual addition of cascades has on the correlation functions.

  \subsection{Event selections}
  % \tp{Your usage of the term `selection' and in particular the plural follows a bad ALICE convention, a jargon, which is in fact incorrect language. Selection is in the first place a process, by which you select something. This is done by using certain criteria (`selection criteria') or cuts. In ALICE people often use `selections' to denote a number of cuts. That is not correct. One selection (i.e. the process) can use several criteria or cuts. But several selections would mean that you do several processes of selecting something, each with possibly several cuts, so one rarely would use that. If you really mean the criteria, use `cuts' or `criteria'. 
  % Alternatively, `selection' can also mean a selected group of objects. Then it can be reasonably used in the plural form. You can e.g. have a selection of cascade candidates, which you obtain by using certain selection cuts. You could have one selection, which e.g. has a high chance of being a true cascade. The complement (background) would not be another selection, but rather the rejected candidates. A second selection would require to change your selection cuts. Then you would have two selections of candidates, but that is always the result of the process, and not the criteria. I may sound picky, but I want to try to fight this where I can ;-) I may note a few explicit examples below, but I urge you to check the consistent use of `selection' throughout.}
  \rs{Thomas mentioned the usage of the terms selections/criteria/cuts, I agree and will try to edit the text everywhere to be more consistent. }
  
  \label{sec:eventsel}
    \rs{I've added a small introduction here, and expanded on the criteria.}
    We want to make sure that the events we analyze are relevant and of sufficient quality. To achieve this we reject events that do not meet certain criteria. Before discussing these criteria, we first introduce a few terms related to how we produce collisions and reconstruct events. 
    
    In the LHC, two beams of protons circulate in opposite directions and cross over at Interaction Points (IPs), where detectors such as ALICE are located. These beams are not a continuous stream of protons, but rather consist of multiple ``bunches'' of protons grouped together. The crossovers of the beams at the IPs are orchestrated such that the bunches overlap, increasing the probability of collisions.\footnote{The amount of overlap, or ``leveling'' can be tuned, and in case of ALICE this is actually tuned to a small fraction of the possible maximum to maintain a manageable interaction rate.} A single instance of two bunches crossing each other is called a bunch crossing (BC), in practice this is the smallest temporal interval. Each BC may result in zero, one, or multiple collisions - though the probability of multiple collisions in the same BC is quite low with ALICE data taking conditions. In general there are on the order of ~2-3 thousand bunches per beam. For the reconstruction, the data is split into Time Frames (TF) defined as 32 orbits. One orbit is a single cycle of the beam, taking approximately 90 microseconds, so a TF is about 3 milliseconds. A TF may be considered as the fundamental processing unit. Related to this are ITS ReadOut Frames (ITSROF), the discrete time intervals in which the ITS detector reads out data, which are roughly 5 microseconds. 

    A collision doesn't usually happen at the exact centre of the detector, but rather at some position along the beam axis with a small deviation in the transverse plane. The exact position of the collision is called the primary vertex (PV). The PV is reconstructed based on the tracks of the charged particles produced in the collision, and it is used as a reference point for many calculations. 

    We use the following criteria to select events for our signal. 
    % \tp{Here you use criteria, which is apropriate.}
    Note that the same event selection is applied %\tp{That is an example where it goes wrong. You don't apply selections, you either perform \textbf{a} selection, or you apply selection \textbf{cuts}.}
    when requiring that a generated collision has a matched reconstructed collision that passes event selections, which is the case in our efficiency calculation.

    \begin{enumerate}
      \item \texttt{sel8}: events with the sel8 selection flag, which groups a few criteria together. The main purpose is to check that the timing of the event triggers on either side of the detector is compatible - i.e. the time difference is sufficiently small. A large time difference would indicate that the event is due to for instance beam-gas interactions. On top of this, the sel8 critation also rejects events that are at the edges of a TF or an ITS ROF. These time intervals are treated independently in the reconstruction \cite{OverviewO2reconstruction} and as such cannot handle events that are split across them. 
      \item INEL$>0$: at least 1 central charged track that contributes to the primary vertex reconstruction. This ensures that the collision is ``inelastic'', which roughly translates to a collision where the colliding protons break apart and produce new particles. This is in contrast to ``elastic'' collisions where the protons remain intact and simply scatter off each other. 
      \item $V_z < 10$cm: the primary vertex position along the beam axis must be within 10 cm of the centre of the detector. This ensures that the collision occurs within the region of the detector where we have good coverage. 
      \item \texttt{kNoSameBunchPileup}: as the name suggests, this rejects events with same bunch pile-up. Same bunch pile-up is caused by multiple collisions occurring in the same bunch crossing making it difficult to distinguish which particles belong to which collision. 
    \end{enumerate}
...
\tp{At this point the reader needs to know some background about cascades and their decay topologies. See comment above - you can do it earlier, but if you want that part to be brief, you may have to add some more detail - e.g. sketches of the topology - here.}\rs{I've added some background on cascades in the theory chapter, and a figure of the decay topology here.}
  In order to study the angular correlations of cascade pairs we first need to reconstruct cascades. As already mentioned in Section \ref{sec:strangehadrons} cascades are reconstructed via their decay chains \cite{PDG2024}
  \begin{align}
  ...
  \subsection{Selection criteria for \texorpdfstring{\Xi}{Xi} reconstruction}

    The cascade candidates are selected based on the criteria summarised in Table \ref{tab:cascsel}. Most selection criteria may be split into two categories: particle identification (PID), and topological selection cuts, both of which are explained in more detail below. An invariant mass selection is applied on the \Lambda\ candidates. This selection criterion is defined as a symmetrical acceptance window around the \Lambda\ mass as listed by the PDG \cite{PDG2024}.

    Besides the selection criteria for \Lambda\ and cascade candidates, we also apply a few basic requirements to the charged tracks that are used to build the pairs. These are generally motivated by ensuring that the track is of sufficient quality to be used in the reconstruction. For example, we require that the track has at least 50 crossed rows \tp{explain this term?} in the TPC, and that the $\chi^2$ per TPC cluster is less than 4. For the ITS we require that the daughter tracks of the \Lambda\ have at least two hits each, but no such requirement is applied to the bachelor track. In all three cases the $\chi^2$ per ITS cluster has to be less than 36 \mvl{Check whether there is an intrinsic minimum in the tracking (iirc the tracking does standalone track finding for ITS and TPC first and then combines the tracks, so there is probably a 3- or 4- hit minimum. Maybe there is a 'afterburner' pass with a less strict requirement}\rs{There's a 2hit minimum incase of the V0 daughters, but no minimum on the bachelor. I've changed it accordingly}. On top of this we only use tracks within $|\eta| < 0.8$ and with a transverse momentum $\pt > 0.15$ GeV/$c$, as these are the kinematic ranges in which the ALICE detector has a good efficiency. 
    ...
    \begin{table}[ht]
      ...
      \begin{tabular}{|c|c|}
        ...
        \hline
        Cascade $\cos(\text{PA})$ & $> 0.9925$ \\
        \hline
        V0 radius & $> 0.55$ \\
        \hline
        Cascade radius & $> 1.5$ \\
        \hline
        V0 mass window & $< 11.5$ MeV/$c^2$\\
        \hline
      \end{tabular}
  ...
  \subsection{PID}
    The PID selection is based on the energy loss d$E$/d$x$ in the TPC being within 5$\sigma$ of the expected value for (anti-)protons, pions, and kaons. The $\sigma$ refers to the width of the band around the curve expressed in standard deviations, so a 5$\sigma$ cut means that only one in a few million true particles would fall outside the band. As the 5$\sigma$ bands around the Bethe-Bloch curves may overlap, it is possible for a track to be consistant with multiple PID hypotheses. In this case we do not reject any of the hypotheses but simply accept all of them. For higher momentum tracks the bands start to overlap more significantly which means there is very little to no separation power between different PID hypotheses. For these tracks the PID selection has effectively no impact other than removing tracks with a very off d$E$/d$x$ measurement.
    
    % \rs{add energy loss plots? or maybe we should put those in the detector chapter? edited: I've now added them to the detector chapter.}
  ...
  \subsection{Topological selection criteria}
...
\tp{Here you could make use of a sketch of the geometry of the decay to illustrate the definition of some cut variables. It would also be useful, in particular in light of a later usage of the variables in other contexts, to define proper variable names like for the DCA of a daughter $i$: $d_i$, for the DCA between to daughters:  $d_{12}$, and for the DCA of the V0: $d_{V0}$, and so on for the other variables. }\rs{I've added a figure of the decay topology, but I'm not sure giving the variables names is necessary. I'd rather keep them verbose/descriptive, so it's immediately clear what they refer to. I don't really mention them later anyway.}

    % We may leverage 
    We may put constraints the geometry of the decay to eliminate candidates that are inconsistent with physical expectations. For instance, we would expect that the minimum distance between daughter tracks from the same mother tends to zero. Experimentally this is never exactly the case, but we can put a maximum value on this distance to reject candidates daughter pairs that do not have a single point (read: decay vertex) in common. A sketch of the cascade decay topology is shown in Figure \ref{fig:cascadetopology}, this could prove useful in visualising the different quantities we describe below. 

    \begin{figure}[htbp!]
      \centering
      \includegraphics[width=.8\textwidth]{figures/Methodology/CascadeDecayTopo.pdf}
      \caption{Schematic representation of the cascade decay topology, taken from \cite{CascadeDecayTopology}. It shows the decays of \Xi\ baryons (top) and \Omega\ baryons (bottom) in the presence of a magnetic field, as is the case in the ALICE detector. }
      \label{fig:cascadetopology}
    \end{figure}

    \subsubsection{Distance of closest approach (DCA)}
      In the selection of weak decay candidates, two different DCA values are used: the DCA between two daughters, and the DCA between a daughter and the PV. The former is used to ensure that the two daughters are close enough to each other to be consistent with originating from a common secondary vertex, while the latter is used to diminish the combinatorial background by requiring that the daughters are not consistent with originating from the PV.
  ...
  \subsection{Signal selection}
    ...
    After all these selection cuts are applied, we turn to the invariant mass distribution of the cascade candidates in Figures \ref{fig:XiMassLow}, \ref{fig:XiMassMid}, \ref{fig:XiMassHigh}. These show a well-defined peak around the nominal mass of the \Xi\ baryon (1321.71 MeV/$c^2$ \cite{PDG2024}) on top of the combinatorial background. Candidates far away from the mass peak are likely to be false positives, so we apply an invariant mass selection to only keep candidates within the signal region. 
    ...
    \begin{figure}[ht!]
      ...
      \begin{subfigure}[t]{.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Methodology/mass/XiMinus1.0_2.5.pdf}
      \end{subfigure}
      \hfill
      \begin{subfigure}[t]{.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Methodology/mass/XiPlus1.0_2.5.pdf}
      \end{subfigure}
      \caption{$\Xi^-$ (left) and $\Xi^+$ (right) invariant mass for $1.0 < p_T < 2.5$ GeV/c, fitted by a double gaussian on top of a second order polynomial. The dashed vertical lines represent the 3, 4, and 10 $\sigma$ levels on either side of the mass peak. }
      \label{fig:XiMassLow}
    \end{figure}
    ...
    \begin{figure}[ht!]
      ...
      \begin{subfigure}[t]{.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Methodology/mass/XiMinus2.5_4.0.pdf}
      \end{subfigure}
      \hfill
      \begin{subfigure}[t]{.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Methodology/mass/XiPlus2.5_4.0.pdf}
      \end{subfigure}
      \caption{$\Xi^-$ (left) and $\Xi^+$ (right) invariant mass for $2.5 < p_T < 4.0$ GeV/c, fitted by a double gaussian on top of a second order polynomial. The dashed vertical lines represent the 3, 4, and 10 $\sigma$ levels on either side of the mass peak. }
      \label{fig:XiMassMid}
    \end{figure}
    ...
    \begin{figure}[ht!]
      ...
      \begin{subfigure}[t]{.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Methodology/mass/XiMinus4.0_8.0.pdf}
      \end{subfigure}
      \hfill
      \begin{subfigure}[t]{.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/Methodology/mass/XiPlus4.0_8.0.pdf}
      \end{subfigure}
      \caption{$\Xi^-$ (left) and $\Xi^+$ (right) invariant mass for $4.0 < p_T < 8.0$ GeV/c, fitted by a double gaussian on top of a second order polynomial. The dashed vertical lines represent the 3, 4, and 10 $\sigma$ levels on either side of the mass peak. }
      \label{fig:XiMassHigh}
    \end{figure}

\section{Sideband subtraction \& trigger normalisation}
  \label{sec:sideband}
  To estimate the scale of the combinatorial background under the mass peak, we first calculate the purity $\eta$ of the selected cascade sample. This is done with the counts of the invariant mass histogram $S$ and the area under the fitted background polynomial $B$, both within the signal region:
  \begin{align}
    \eta = \frac{S - B}{S}. \label{eq:purity}
  \end{align}
  The purity is quite high, which indicates that there is limited contamination from the combinatorial background in our selected cascade sample. Nevertheless, we want to estimate the effect of this background on the correlation functions. To do this we determine the correlation function between two cascades where the trigger is taken from one of the sideband regions defined as $[\mu - 10\sigma, \mu - 4\sigma]$ and $[\mu + 4\sigma, \mu + 10\sigma]$, while the associate is taken from the signal region as usual. The correlation functions obtained from the two sidebands are then averaged to obtain the sideband correlation function $C_\text{SB}$. This is done for each of the four sign-combinations, and subtracted from the corresponding signal correlation function. The sideband correlation functions are presented in Figure \ref{fig:CSB}, averaged over OS and SS combinations. 

  \begin{figure}[ht]
    \centering
    \begin{subfigure}[t]{.49\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/Methodology/SidebandOS.pdf}
      % \caption{Sideband correlation function for OS combinations}
      % \label{fig:CSBOS}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{.49\textwidth}
      \centering
      \includegraphics[width=\textwidth]{figures/Methodology/SidebandSS.pdf}
      % \caption{Sideband correlation function for SS combinations}
      % \label{fig:CSBSS}
    \end{subfigure}
    \caption{Sideband correlation functions for (a) OS and (b) SS combinations. They are not averaged over the number of triggers, as we want to perform the subtraction before applying the trigger normalisation.}
    \label{fig:CSB}
  \end{figure}
...
\label{sec:corrections}
  ...
  \label{sec:efficiency}
    ...
    And because the trigger efficiency corrections cancels out in the equation, that leaves only the associate efficiency correction. As the existance of an associate cascade implies the existence of a trigger, and therefore the event being reconstructed, we only need to correct for the associate cascade reconstruction efficiency \textit{given that} the event is reconstructed.
    ...
    With $N_\text{rec.}$ ($N_\text{gen.}$) the number of reconstructed (generated) cascades, we calculate the reconstruction efficiency $\epsilon$ as
    \begin{align}
      \epsilon(\pt, \eta) = \frac{N_\text{rec.}(\pt, \eta)}{N_\text{gen.}(\pt, \eta) | \text{rec. event}}. \label{eq:efficiency}
    \end{align}
% \tp{ the equation is ugly due in particular to the subscript of the denominator, Avoid such long subscripts. Define a different variable and describe it properly in the text.}
    The inverse of these efficiencies are applied as a weight to each cascade when constructing the pairs, as well as to the trigger normalizations. The efficiencies as function of \pt\ and \eta\ are presented in Figure \ref{fig:eff}. Projections onto the \pt\ axis are presented in Figure \ref{fig:effPt}. 
    ...
    \begin{figure}[ht]
      ...
      \begin{subfigure}[t]{.49\textwidth}
        ...
        % \label{fig:XiPlusEff}
      \end{subfigure}
      \caption{Reconstruction efficiencies of the $\Xi^-$ (left) and $\Xi^+$ (right) as a function of \pt\ and \eta.}
      \label{fig:eff}
    \end{figure}
\tp{Need to clean up the figures and improve the caption. On the caption: this is not the efficiency correction, I assume, but the efficiency, right? You should write a little more text, e.g. for figure 4.3 "Efficiency for the studied cascades as a function of \y\ and \pt\." Then you have titles on top of each figure and captions for each subfigure, which say something similar again. I would get rid of the plot titles anyway - those are good for plots in a presentation, but not for a thesis or paper. You could also leave out the subfigure caption, because the only info you need in addition to the overall caption is which cascades you are showing. You could do that more efficiently with a label of the cascade symbol (e.g. $\Xi^-$) inside the plot frame. Similar for figure 4.4, only the caption should read "Efficiency for the studied cascades as a function of \pt\."}\rs{I've cleaned up the captions, the updated figures as function of pt,eta with proper text sizes will follow}
    ...
    \begin{figure}
      ...
      \end{subfigure}
      \caption{Projections of the $\Xi^-$ (left) and $\Xi^+$ (right) reconstruction efficiencies onto the \pt\ axis.}
      \label{fig:effPt}
    \end{figure}
  ...
  \label{sec:ME}
    ...
    The limited acceptance of the ALICE detector does not only affect the single-particle efficiency (as can be seen in Fig. \ref{fig:eff} from the drop in effciency at larger $|\eta|$), it also impacts the correlation functions depending on the angular differences \dphi, \dy\ between the two cascades. For example, a pair with a \dy\ corresponding roughly to the ends of the ALICE detector has a very low probably of being detected - there is only ``one" way the cascades may be oriented such that both cascades fall into the detector acceptance: both at the respective ends of the detector. On the other hand, a pair with negligible angular difference does not suffer from this, if the trigger falls in the detector acceptance then so does the associate. 
    
    One cannot correct for these effects on a single-particle level so we do this on the two-particle level by means of a mixed events correction. 
    % By calculating the correlation function of two cascades from different events we can determine the effect on the correlation function that is purely due to detector effects. 
    \tp{I think this is not correct. E.g. limited two-track separation power is a possible detector effect, but is not accounted for by event mixing.  better description may be that this includes all single-particle effects (from both detector and analysis) that affect two-particle kinematic variables.}\rs{I agree, fixed it and elaborated a bit more.} 
    By calculating the correlation function of two cascades from different events we can isolate the effects on the correlation function that have no physical origin, and therefore have to be due to detector limitations or analysis procedure. Note that this is not an exhaustive correction: for example, two tracks that are very close together may not be accurately reconstructed due to the limited two-track separation power of the detector. This is not captured by the mixed events correction. 
    It is important to realise that we do still apply the single-particle efficiency correction while computing the correlation function for mixed events. This is because the uncorrected correlation function in mixed events is essentially a convolution of single-particle and two-particle efficiencies, but we want the mixed events correction to \textit{only} encapsulate the two-particle efficiency. 
    ...
    \end{align}
    where $\xi$ is a normalisation factor. Ideally the mixed event correction is normalised such that it is 1 at $(\dphi,\dy) = (0,0)$. However, scaling the entire distribution such that the central four bins\footnote{The binning is such that the $(0,0)$ coordinate corresponds to a bin edge in both dimension.} average to 1 is quite sensitive to statistical fluctuations in these bins, which is a problem because the statistics of the mixed events correlation function are quite limited. To remedy this we normalise by the average in the region defined by $|\y| < 0.1$ and $-\pi/2 < \dphi < \pi/2$, which corresponds to two bins in \dy\ and 10 bins in \dphi. 
    ...
    \end{align*}
    where $C^\text{T,A}_\text{SE}$ is the signal correlation function obtained from same events. The mixed events correction for \Xi\ - \Xi\ correlations are presented in Figure \ref{fig:ME2D}. The 1D projections are presented in Figure \ref{fig:ME1D}, where the normalization is no longer to 1 but to the average over the other dimension. 
    \tp{This is unclear. First of all, you probably normalize the correlation function to a particular value averaged over the other dimension. (The average \textbf{of} the other dimension, as you now write, would be e.g. $<\Delta y> = 0$ for the left plot.) And it is also not obvious how you normalise it. Is the integral of the distribution shown set to be equal to the average value of the integral, or is this normalisation done bin by bin?}\rs{changed `of other dim.' to `over other dim.'. Also added some clarification on the normalisation procedure.}

    \begin{figure}[ht]
      \centering
      \includegraphics[width=.8\textwidth]{figures/Methodology/ME/XiXi.pdf}
      \caption{\Xi\ - \Xi\ correlation function from mixed events as a function of \dphi\ and \dy. The correlation function is scaled such that the average of the central region defined by $|\dy| < 0.1$ and $|\dphi| < \pi/2$ is equal to 1.}
      \label{fig:ME2D}
    \end{figure}
    ...
    \begin{figure}[ht]
      ...
      \begin{subfigure}[t]{.49\textwidth}
        ...
        % \label{fig:MEdY}
      \end{subfigure}
      \caption{Projections of \Xi\ - \Xi\ correlation function from mixed events onto \dphi\ (left) and \dy\ (right). 
      \tp{Also in figure 4.6 (and effectively in several other occasions), please retune the figure. I would try to avoid the subfigure captions and instead adress the subfigures in the main caption with an appropriate label (left/right, or a/b, if you prefer). Add axis labels. I would also always start with a sentence saying what is plotted as a function of what, so here: \Xi\ - \Xi\ distribution from mixed events as a function of \dphi\ (left) and \dy\ (right). }\rs{fixed caption, figure retuning will follow}}
      \label{fig:ME1D}
    \end{figure}
