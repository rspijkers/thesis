...
\section{Datasets etc.}
  ...
  \label{sec:eventsel}
    ...
    Note that these same event selections are applied \tp{That is an example where it goes wrong. You don't apply selections, you either perform \textbf{a} selection, or you apply selection \textbf{cuts}.}
...
\tp{At this point the reader needs to know some background about cascades and their decay topologies. See comment above - you can do it earlier, but if you want that part to be brief, you may have to add some more detail - e.g. sketches of the topology - here.} 
  ...
  \subsection{Topological selections \tp{selection criteria}}
...
\tp{ the equation is ugly due in particular to the subscript of the denominator, Avoid such long subscripts. Define a different variable and describe it properly in the text.}
    The inverse of these efficiencies are applied as a weight to each cascade when constructing the pairs, as well as to the trigger normalizations. The efficiencies as function of \pt\ and \y\ are presented in Figure \ref{fig:eff}. Projections of the \tp{something missing?} onto the \pt\ axis are presented in Figure \ref{fig:effPt}. 
    ...
    \begin{figure}[ht]
      ...
      \label{fig:eff}
    \end{figure}
\tp{Need to clean up the figures and improve the caption. On the caption: this is not the efficiency correction, I assume, but the efficiency, right? You should write a little more text, e.g. for figure 4.3 "Efficiency for the studied cascades as a function of \y\ and \pt\." Then you have titles on top of each figure and captions for each subfigure, which say something similar again. I would get rid of the plot titles anyway - those are good for plots in a presentation, but not for a thesis or paper. You could also leave out the subfigure caption, because the only info you need in addition to the overall caption is which cascades you are showing. You could do that more efficiently with a label of the cascade symbol (e.g. $\Xi^-$) inside the plot frame. Similar for figure 4.4, only the caption should read "Efficiency for the studied cascades as a function of \pt\."}
  ...
  \label{sec:ME}
    ...
    One cannot correct for these effects on a single-particle level so we do this on the two-particle level by means of a mixed events correction. By calculating the correlation function of two cascades from different events we can determine the effect on the correlation function that is purely due to detector effects. \tp{I think this is not correct. E.g. limited two-track separation power is a possible detector effect, but is not accounted for by event mixing.  better description may be that this includes all single-particle effects (from both detector and analysis) that affect two-particle kinematic variables.} It is important to realise that we do still apply the single-particle efficiency correction while computing the correlation function for mixed events. This is because the uncorrected correlation function in mixed events is essentially a convolution of single-particle and two-particle efficiencies, but we want the correlation function to \textit{only} encapsulate the two-particle efficiency. 
