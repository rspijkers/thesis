...
\section{QCD}
  ...
  \subsection{Asymptotic freedom}
    ...
    In the context of high energy particle physics experiments at the LHC, perturbative QCD is applicable in the initial hard scattering between 2 partons from the colliding protons, while the subsequent parton showering \mvl{the parton shower mostly perturbative} and hadronisation are mainly non-perturbative processes.  
...
\section{Hadronisation}
  \subsection{Lund string model}
    ...
    A massive quark-antiquark pair cannot be created at the exact same space-time point \mvl{the same point as what?} as this would violate energy conservation. \mvl{The following is a microscopic picture/description of string breaking; mention that explicitly?}Instead, a virtual \qqbar\ pair with $E_q = E_{\overline{q}} = 0$ is created with imaginary longitudinal momentum along the string $p_L = \pm i\sqrt{\pt^2 + m^2}$. As the quark and antiquark move along the string, they gain energy from the string tension until their longitudinal momentum becomes real (zero). At this point they tunnel into real quarks, which immediately gives us a handle on the probability of this happening\mvl{Not sure I understand teh relation between this equation and the picture in the prev sentence. Clarify?}: \cite{StringModelSlides} 
    \begin{align}    
  ...
  \subsection{Extensions of the string model}
    ...
    \subsubsection{Junctions}
      From SU(3) algebra, one can calculate the superpositions of colour states of two uncorrelated partons. Taking the example of two gluons \mvl{why are two gluons relevant to consider? Is this part of teh 'internal structure' of strings? Or is this relevant for events with two gluon jets?}, one gets 
      \begin{align}
        8 \otimes 8 = 27 \oplus 10 \oplus \overline{10} \oplus 8 \oplus 8 \oplus 1. \label{eq:gluongluon}
      \end{align}
      Here the 27 represents the leading colour configuration - each gluon carrying two units of colour charge. The other lower order \mvl{is 'lower order' the correct terminology?} terms correspond to states where colour states overlap, leading to lower net colour charge. The singlet case corresponds to two gluons having exactly opposite colour charges. Simple probability arguments show that the leading colour configurations, while most probable, only account for 27/64 $< 50\%$ \mvl{This seems to assume that all of the 64 states are equally probable; that makes sense. Does that mean that the first 27 are simply the largest group?} of all possible configurations. In other words, sub-leading colour configurations are far from negligible. 

      One of the key characteristics of these sub-leading colour configurations is the possibility to create junctions: topologies where 2 colour charges are allowed to combine into a single anti-colour charge \mvl{maybe: 'which can combine with a third quark to create a color-neutral configuration'}. The accompanying 2 anti-colours must then also combine into a single colour charge to conserve colour and baryon number. The resulting string topology looks like a Y-shape, with 3 string pieces meeting at a single point. This string may break into (multiple) \qqbar\ pairs, allowing the combined quarks to hadronise into baryons. This process is schematically depicted in Fig. \ref{fig:junction} - note that more complex topologies involving multiple junctions are also possible. 

      \pc{I feel you should be a bit more explicit when you discuss about the quark vs the gluon color states and how the latter combine to form junctions and how the former combine to form colour singlet hadrons (baryons in this case). The gluon colors is indeed connected to the one of the quarks, but the discussion (I feel) jumps from one to the other w/o a clear distinction.}
